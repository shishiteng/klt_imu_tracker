...
void CImageAlign::WarpBackImage(IplImage* I_whole, IplImage* I, CvRect r, CvMat* motion)
{
    ...
    IppiSize srcSize = {I_whole->width, I_whole->height}; 
    IppiRect srcRoi = {0, 0, I_whole->width, I_whole->height};
    IppiSize dstSize = {I->width, I->height}; //不确定是否正确
    //datatype
    //ceoffs 
    IppiBorderType borderType = ippBorderConst;
    IppiWarpDirection direction = ippWarpBackward;
    int numChannels = 1;
    Ipp64f pBorderValue[3] = {255,255,255};
    int smoothEdge = 0;
    IppiWarpSpec* pSpec = (IppiWarpSpec*)ippsMalloc_8u(0);
    //get size
    ippiWarpPerspectiveGetSize(srcSize, srcRoi, dstSize, ipp32f, coeffs, ippLinear, direction, borderType, &specSize, &initSize);

    // init
    pSpec    = (IppiWarpSpec*)ippsMalloc_8u(specSize);
    pInitBuf = ippsMalloc_8u(initSize);
    ippiWarpPerspectiveLinearInit(srcSize, srcRoi, dstSize, ipp32f, 
				  coeffs, direction, numChannels,
				  borderType, pBorderValue, smoothEdge, pSpec);
    // Work buffer size
    ippiWarpGetBufferSize(pSpec, dstSize, &bufSize);

    // run
    IppiPoint dstOffset = { 0, 0 };
    Ipp8u* pBuffer = ippsMalloc_8u(bufSize);
    ippiWarpPerspectiveLinear_32f_C1R(src, srcStep, dst, dstStep,dstOffset, dstSize, pSpec, pBuffer);
...
#if 0
	IppiFilterRow   (T + offset, widthStep, Gx + offset, widthStep, roi, kernel, 3, 1);
	IppiFilterColumn(T + offset, widthStep, Gy + offset, widthStep, roi, kernel, 3, 1);

        ippiFilterBoxBorder_32f_C1R(const Ipp32f* pSrc, int srcStep, Ipp32f* pDst, int dstStep,
					IppiSize roiSize,
					IppiSize maskSize, IppiBorderType border,const Ipp32f* borderValue, Ipp8u* pBuffer))
        ippiFilterBoxBorderGetBufferSize(IppiSize roiSize, IppiSize maskSize, IppDataType dataType, int numChannels, int* pBufferSize))
#else

	int bufSize = 0;
	int numChannels = 1;
	IppiSize maskSize = {0, 0};
	ippiFilterBoxBorderGetBufferSize(roi, maskSize, ipp32f, numChannels, &bufSize);

	Ipp8u* pBuffer = ippsMalloc_8u(bufSize);
 	IppiBorderType borderType = ippBorderConst;
        Ipp32f pBorderValue[3] = {255,255,255};
        ippiFilterBoxBorder_32f_C1R(T + offset, widthStep, Gx + offset, widthStep, roi, maskSize, borderType,pBorderValue, pBuffer);
        ippiFilterBoxBorder_32f_C1R(T + offset, widthStep, Gy + offset, widthStep, roi, maskSize, borderType,pBorderValue, pBuffer);
#endif
	int mode = 1;
	if (mode == 0) {
		//! W = |Gx| + |Gy|
